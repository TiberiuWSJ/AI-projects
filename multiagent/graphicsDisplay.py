...
class PacmanGraphics:
    ...
    def drawWater(self, waterMatrix):
        waterImages = []
        color = WATER_COLOR
        for xNum, x in enumerate(waterMatrix):
            if self.capture and (xNum * 2) <= waterMatrix.width:
                color = TEAM_COLORS[0]
            if self.capture and (xNum * 2) > waterMatrix.width:
                color = TEAM_COLORS[1]
            imageRow = []
            waterImages.append(imageRow)
            for yNum, cell in enumerate(x):
                if cell:  # There's food here
                    screen = self.to_screen((xNum, yNum))
                    dot = square(screen,
                                 WATER_SIZE * self.gridSize,
                                 WATER_COLOR)
                    imageRow.append(dot)
                else:
