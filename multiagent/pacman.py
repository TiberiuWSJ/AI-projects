...
class PacmanRules:
    ...
    def applyAction(state, action):
        ...
        PacmanRules.counter = PacmanRules.counter - 1
        next = pacmanState.configuration.getPosition()
        nearest = nearestPoint(next)
        x,y = nearest
        if state.data.water[x][y]:
            PacmanRules.PACMAN_SPEED = 0.5
            PacmanRules.counter = 1
        else:
            PacmanRules.PACMAN_SPEED = 1

        vector = Actions.directionToVector(action, PacmanRules.PACMAN_SPEED)
        # Update Configuration
        if PacmanRules.counter == 0:
            pacmanState.configuration = pacmanState.configuration.generateSuccessorS(
                vector)
        else:
